const CONFIG = {
  weddingDate: '2026-06-07T09:00:00+07:00',
  calendarTitle: 'The Wedding Of Heni & Rijal',
  calendarLocation: 'Kp. Cisaga RT 018 RW 006, Desa Cisaga, Kec. Cibogo, Kab. Subang, Prov. Jawa Barat',
  calendarDetails: 'Undangan pernikahan Heni & Rijal',
  whatsappNumber: '6285295555565',
    ...
    events: {
      onStateChange(event) {
        if (event.data === YT.PlayerState.ENDED) {
          youtubePlayer.seekTo(CONFIG.youtubeStart);
...
const guestParam = params.get('to') || params.get('nama') || params.get('name') || params.get('guest') || params.get('tamu');
if (guestParam) guestName.textContent = decodeURIComponent(guestParam.replace(/\+/g, ' '));
...
document.getElementById('calendarBtn').addEventListener('click', e => {
  e.preventDefault();
  const start = '20260607T020000Z';
  const end = '20260607T090000Z';
...
document.getElementById('rsvpForm').addEventListener('submit', e => {
  ...
  const text = `Halo, saya ${name}.%0AKonfirmasi Kehadiran: ${attendance}.%0AUcapan: ${message || '-'}%0A%0AUndangan: ${window.location.href}`;
...
const COMMENT_STORAGE_KEY = 'heniRijalWeddingComments';
